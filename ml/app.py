class Title_Model():
    def __init__(self, model_path=r'F:\WorkSpace\Projects\Mapsmap-hackathon\ml\Final_Model\bert',
                 ...
                 preprocess_path=r'F:\WorkSpace\Projects\Mapsmap-hackathon\ml\Final_Model\Preprocess', n_labels=674):
        import keras
    ...
    def decode(self, X, n='all'):
        keys = list(self.mlb.inverse_transform(
            self.np.ones(self.n_labels).reshape(1, -1))[0])
        ret = []
        if n == 'all':
            n = self.n_labels
        for values in X:
