...
const Tags = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://mapsmap.herokuapp.com/v1/api/tag?tagId=${tagId}`)
      .then((res) => {
  ...
  const Qncards = () => {
    if (tagQns == {}) {
      ...
      const cards = tagQns.map((qn, n) => {
        ...
        return (
          <div className="card mt-3 mb-3" key={n}>
            <div className="card-head">{qn.question}</div>
            <hr />
            <div className="card-body">{qn.body}</div>
            <div>
              Click here to{" "}
